...
#define F(x) 	(A_coeff_curr*x*x*x)+(B_coeff_curr*x*x)+(C_coeff_curr*x)+D_coeff_curr
#define FD(x) 	(3.0*A_coeff_curr*x*x)+(2.0*B_coeff_curr*x)+(C_coeff_curr)
...
__global__ void nrpol3_kernel(float *A_coeff, float *B_coeff, float *C_coeff, float *D_coeff, float *x0_in, float *root, int size, float err_thresh)
{
	...
	{	
		...
		float A_coeff_curr 	= A_coeff[idx];
		float B_coeff_curr 	= B_coeff[idx];
		float C_coeff_curr 	= C_coeff[idx];
		float D_coeff_curr 	= D_coeff[idx];
		float x0_curr  		= x0_in[idx];
